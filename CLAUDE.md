...
**Job 2 — Produce** (cron: `*/15 * * * *`, every 15 min, action: `produce`):
Picks the best topic from the queue, self-chains through 4 production stages:
  ...
  - **Write** (~90s, temp 0.5, 16K max tokens): Sonnet is ALWAYS primary writer — Gemini/Grok are fallback only (Gemini writes dead wiki-style prose that ignores voice instructions). Brand voice formula in prompt: 60% exceptional journalism, 20% Bill Maher, 15% Hitchens, 15% Sam Harris. Anti-wiki rules with measurable targets (max 3 sentences/paragraph, 1 sub-8-word sentence per 3 paragraphs, 6+ "you", 2+ everyday analogies). Mandatory `selfAudit` JSON field: writer must report analogies used, editorial positions taken, follow-the-money angle, Bill Maher moment — if these can't be filled, article fails. Per-article editorial directives: "who profits from the status quo?", "take at least 2 editorial positions", "earn the Bill Maher moment". Epistemic integrity framework: evidence hierarchy, dogma traps list, contrarian checkpoint, follow-the-money. Zero fabrication rule — only cite research data. Mandatory Sources section with `id="sources"`. Variable word counts per archetype (1,200–2,400). `model_used` tracked.
  ...
  - **QC + Publish** (~60s): **Gemini → Sonnet** fallback chain (NOT Grok — different model from independence reviewer, prevents same-model rubber-stamping). Receives mechanical voice audit (code-generated metrics: banned phrases, "you" count, paragraph length, rhetorical questions) + independence review flags. Gates on voice quality — auto-revise on banned phrases or missing editorial opinion, auto-kill on 3+ banned phrases with zero opinion. Default-deny: missing/truncated QC decision = "revise" (not silent publish). Description truncation guard at publish: validates punctuation + length, tries 3 fallback sources. Illustration generation runs in parallel with QC (saves 30-60s). Max 1 revision. OpenAI GPT Image generates illustration. Commits .astro + .json to GitHub. Featured rotation with early-exit optimization.

**Self-chaining**: each stage triggers the next via fire-and-forget HTTP POST with 10s retry on failure. 15-min cron acts as safety net for dropped chains. Concurrency guard (5-min window) prevents duplicate runs.
...
**Fallback chain**: Every model call goes through `generateWithFallback()` or has explicit try/catch with provider fallback. Research falls back Claude → Gemini. Scout structuring uses full Sonnet → Grok → Gemini chain. QC uses Gemini → Sonnet chain (webSearch disabled).
**Mechanical voice audit**: `auditVoiceQuality()` runs code (not AI) on article HTML before QC. Scans 30+ banned phrases, counts "you" usage (min 6), checks paragraph length (max 3 sentences), short sentence ratio, rhetorical question count (max 2). Feeds hard metrics into QC prompt. QC auto-revises on failures, auto-kills on 3+ banned phrases with zero editorial opinion.
**Truncation guards**: `parseClaudeJSON` Step 3 repair now logs warnings. Description validated at write stage and publish stage — must end with punctuation and be ≥80 chars, with 3 fallback sources. Default-deny on QC: missing/unrecognized decision = "revise".
