-- Hybrid pipeline: pause at editor_approved for human writing with Opus.
-- The dispatch function no longer auto-dispatches editor_approved → stage-write.
-- Instead, articles wait at editor_approved until the user submits via admin UI.
-- The user writes with Opus (Max subscription), pastes the article, and the pipeline
-- resumes from "written" → independence review → QC → publish.
...
BEGIN
  ...
  -- NOTE: editor_approved is EXCLUDED — articles pause there for human writing.
  -- The user writes with Opus via Claude Mac/Code, then submits via admin UI,
  -- which sets status to "written". Pipeline resumes from there.
  FOR v_status, v_function_name IN
    SELECT s, f FROM (VALUES
      ...
      -- ('editor_approved', 'stage-write'),  -- REMOVED: human writes with Opus
