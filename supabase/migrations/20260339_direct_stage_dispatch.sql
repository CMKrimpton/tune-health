-- Replace the orchestrator edge function with a SQL function that dispatches
-- stage functions directly via pg_net. Each stage gets its own independent
-- HTTP invocation with its own timeout — no shared connections.
...
DECLARE
  ...
  v_topic record;
BEGIN
  v_base_url := current_setting('app.settings.supabase_url');
  v_service_key := current_setting('app.settings.service_role_key');
  v_stale_threshold := now() - interval '5 minutes';

  -- Step 1: Clean up stale runs (>5min in active status)
  -- Only reset if status is STILL the stale value (atomic CAS)
  ...
  SET status = CASE
    ...
    ELSE 'failed'
  END,
  error = CASE WHEN NOT (research_data ? 'topic' OR research_data ? 'keyFindings' OR research_data ? 'candidates' OR research_data ? '_editorBrief' OR research_data ? '_article') THEN 'Timed out (stale run)' ELSE null END,
  ...
  IF EXISTS (
    ...
    RETURN; -- Another stage is actively running
  END IF;

  -- Step 3: Find highest-priority article and dispatch the right stage
  ...
  LOOP
    ...
    IF v_log_id IS NOT NULL THEN
      -- Dispatch via pg_net (fire-and-forget, independent invocation)
  ...
  IF v_topic.id IS NOT NULL THEN
    ...
    -- Dispatch stage-research via pg_net
    PERFORM net.http_post(
...
-- Update the cron to call the SQL function directly (not the orchestrator edge function)
