...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const [astroBlob, jsonBlob] = await Promise.all([
      createBlob(apiBase, headers, astroContent),
      createBlob(apiBase, headers, jsonContent),
    ]);

    // 4. Create a new tree with the two files
